...
- 字符型：`char(2字节)`
- 布尔型：`boolean(1字节)` // 注：boolean数据值确实只占1字节，但boolean对象本身的大小没被强制规定（不考）
- 变量名指向具体的数值。
- 基本数据类型存储在栈上。
### 引用数据类型：
- 类、接口、**数组**、枚举、注解（描述代码的结构或行为）、包装类（将基本数据类型转换为对象的类）、`String`字符串、哈希表......
- 引用数据类型的默认值为`null`，包括数组和接口。
- 变量名指向的是存储对象的**内存地址**，在**栈**上。
- 内存地址指向的对象存储在**堆**上。
___
## 常量的定义：
...
| float   | 0.0f     | 4 字节 |
| double  | 0.0      | 8 字节 |
- 在进行金融计算或需要精确小数计算的场景中，可以使用 `BigDecimal` 类来避免浮点数舍入误差。`BigDecimal` 可以表示一个**任意大小**且**精度完全准确**的浮点数。
- 变量有**作用域限制**
- **同一作用域**内，变量不可重复定义
- **自动类型转换**：一般是小范围转大范围。比如：`double d = 1000;`
	- 补充：自动类型转换并不局限于小类型到大类型，也可以是大类型到小类型，只要值在小类型的取值范围内，编译器就会自动帮我们做隐式转换。比如`byte b = 50;`，编译器自动将int值转换成了byte值
...
- **数组的创建格式**：`数据类型[] 数组名 = new 数据类型[元素个数或数组长度]`
	- C风格的`数据类型 数组名[] = new 数据类型[元素个数或数组长度]`也行，不推荐。
``` java
// 举个栗子：
// 1.声明与构造分开：
int[] x;         // 声明
...
- **与数组有关的常见错误**：
	...
	- 静态初始化时不能定义数组长度！
	- **数组复制（深拷贝）** 只能使用`Arrays.copyOf()`或者`System.arraycopy()`进行复制，否则复制前后指向的是同一个数组对象！！！！！！！！！！！！！！！
## 多维数组就不在此介绍了
## Arrays类：
| 序号  | 方法和说明                                                                                                                                                                                                                                         |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
- 格式：**数据类型...参数名称**
``` java
// 可变参数是Java的一个语法，本质上还是基于数组的实现：
void foo(String... args);
// 两者完全等同。
void foo(String[] args);
```
- 可变参数的作用：
	- 传输参数非常灵活，方便。可以**不传输参数**，可以传输**1个或者多个**，也可以传输**一个数组**
	- 可变参数在方法内部本质上就是一个**数组**。
- 可变参数的注意事项： 
	1. 一个形参列表中可变参数只能有**一个**
	2. 可变参数必须放在形参列表的**最后**
...
```
### 一个类可以包含以下类型变量：
- **局部变量**：在**方法、构造方法或者语句块**中定义的变量被称为局部变量。
	- 变量声明和初始化都是在方法中，方法结束后，变量就会自动销毁。
	- 局部变量无初始值，需先赋值再使用，作用域仅在定义局部变量的语句块内。
- **成员变量**：成员变量是定义在**类中，方法体之外**的变量。这种变量在创建对象的时候被实例化。
	- 成员变量可以被类中方法、构造方法和特定类的语句块访问。
	- 成员变量有默认的初始化值，作用域在整个类的体内
...
- 特点：
    ...
    - 常见命名规范：全大写加下划线（如 `MAX_DAMAGE`）。
    - 通过类名直接访问。（例：`Weapon.MAX_DAMAGE`）
___
## 对象：
...
## 封装：
### 概念：
- 在面向对象程式设计方法中，封装（英语：Encapsulation）是指一种将抽象性接口的实现细节部分包装、隐藏起来的方法。
...
- **default** (即默认，什么也不写，也叫包私有）: 在**同一包内**可见，不使用任何修饰符。使用对象：类、接口、变量、方法。
- **private** : 在**同一类内**可见。使用对象：变量、方法。 **注意：不能修饰类（外部类）**
- **public** : 对**所有类**可见。使用对象：类、接口、变量、方法
- **protected** : 对**当前类**、**同一包内的其他类**，以及 **不同包中的子类**可见。使用对象：变量、方法。 **注意：不能修饰类（外部类）**
...
```
#### 继承中的构造方法（构造器）：
- 子类是**不继承父类的构造器（构造方法或者构造函数）的**，它只是**调用**（隐式或显式）。
- 子类默认调用父类的**无参构造器**。
- 如果需要被使用的父类构造器**带有参数**，则**必须**在子类的构造器中**显式**地通过`super`关键字调用父类的对应构造器并配以适当的参数列表。
- 如果需要被使用的父类构造器**没有参数**，则在子类的构造器中可以不使用`super`关键字调用父类构造器，系统会**自动调用**父类的无参构造器。
- 如果父类没有无参构造器，则子类也必须显式调用父类构造器（找不到无参super怎么自动调用...）
...
- **“同一名字”** 是多态的基础；
- **“不同环境”** 是多态的前提；
- 与“同名不同参”相关的多态，称为 **“重载（Override）”**，重载可实现一名多用，即同一方法名称具有不同的方法体、形参和返回类型。重载要求**方法名相同，方法形参不同**，返回类型可以不同。
...
- 继承（首先得存在父子关系）
- 重写（父亲留下的家业被孩子发扬光大）
- 父类引用指向子类对象：`Parent p = new Child()`;（提到的是父亲，但干活的是孩子）
### 举个栗子：
``` java
class Shape {
    void draw() {}
...
- **从父到子**： 
	...
	- **注意**： 如果转型后的类型和对象真实类型**不是同一种类型**，那么在转换的时候就会出现 `ClassCastException`异常
...
- 最常用的地方就是**构造器的重载**（还有某位人才在大作业里写的一堆图片显示函数的重载...）。
#### 重载规则：
- 被重载的方法**必须改变参数列表**(参数**个数**、**类型**或**顺序**不一样)；
- 被重载的方法可以改变返回类型；
- 被重载的方法可以改变访问修饰符；
...
```
### 特性：
- 接口**本身**是**隐式抽象**的，声明接口的时候不用使用`abstract`关键字
- 接口中**每一个方法**也是隐式抽象的,接口中的方法会被隐式的指定为 `public abstract`（只能是 `public abstract`，其他修饰符都会报错）。
- 接口中可以含有变量，但是接口中的变量会被隐式的指定为 `public static final` 类型的公共常量（并且只能是 `public`，用 `private` 修饰会报编译错误）。
...
## 枚举类：
### 概念：
- Java 中的枚举类是一个**特殊的类**，一般表示一组常量，比如一年的 4 个季节，一年的 12 个月份，一个星期的 7 天，方向有东南西北等。
- Java 枚举类使用 `enum` 关键字来定义，各个常量使用**逗号** `,` 来分割。（枚举类定义与Kotlin和C++的`enum class`刚好相反，别搞混了！）
...
___
## 常见基本类及API：
### [Object类](https://www.runoob.com/java/java-object-class.html)：
...
```
### [Math类](https://www.runoob.com/java/java-number.html)：
...
- **特点**：
	- **不可实例化**：`System` 类的构造方法是私有的（`private`），因此无法创建其实例。
	- **静态方法和属性**：所有方法和属性均为 `static`，可直接通过类名调用。
- **核心功能**
	1. **标准输入/输出流**：
...
### bigDecimal类：
- `BigDecimal` 是 Java 中用于**高精度十进制计算**的类，位于 `java.math` 包中。它主要用于处理对精度要求极高的场景（如金融计算、货币运算），避免 `float` 和 `double` 的浮点数精度丢失问题。
- **特点**：
	1. **高精度计算**
	    - `float` 和 `double` 是二进制浮点数，无法精确表示某些十进制小数（如 `0.1`），导致计算误差。
	    - `BigDecimal` 以**十进制字符串**存储数值，支持任意精度的计算，避免精度丢失。
	2. **不可变性**
	    - `BigDecimal` 对象是**不可变**的。所有运算（加减乘除）都会返回一个新的`BigDecimal` 对象，原对象不变。
	3. **灵活的舍入模式**
	    - 支持多种舍入方式（如四舍五入、截断等），满足不同业务需求。
---
# 异常及异常的处理
## 异常：
...
- 异常会被层层抛出，若没有书写任何捕捉捕捉和处理异常的方法，异常会从当前方法被抛给调用这个方法的调用者，再从调用者被抛给JVM，由JVM进行默认处理。
- 若书写了`catch`语句进行异常捕捉和处理，如果**异常被完全处理，且未抛出新异常**，则程序会继续运行，否则程序会继续向上抛出异常，直到异常被完全处理或被抛到JVM处。一旦异常被抛到JVM处，JVM在处理完异常之后会立刻终止当前抛出异常的线程，造成**线程死亡（非主线程）**或**程序死亡（主线程）**。
## 语法：
...
## 概念：
- 集合是Java中提供的一种容器，可以用来存储多个**引用类型**的数据。
- 集合由一组接口和对应的实现类组成
- 集合的特点：
	- 长度可变
	- 比数组更灵活
	- 集合不支持基本数据类型，所以集合中存储的对象必须是**引用类型**！！！
- 集合可简单分为以下两类：
	- **Collection 单列集合**，每个元素（数据）只包含**一个值**。
	- **Map 双列集合**，每个元素包含**两个值（键值对）**。
- 四大常用集合：
	- **列表（List）**：元素有序、可重复、有索引
	- **集合（Set）**：元素不重复，无索引
	- **队列（Queue）**
	- **映射（Map）**
...
- 基于`HashMap`实现，不允许有重复元素。
	...
	- `HashMap`的常用方法见[菜鸟教程](https://www.runoob.com/java/java-hashmap.html)
- 允许有`null`值。
...
- 注意：TreeSet集合是**一定要排序**的，可以将元素按照指定的规则进行排序。
	- 对于**数值类型**：Integer , Double，官方默认按照大小进行**升序排序**。 
...
- **运行状态（Running）**：如果就绪状态的线程获取到 CPU 资源，就可以开始执行 **run()**，此时线程便处于运行状态。处于运行状态的线程最为复杂，它可以变为阻塞状态、就绪状态和死亡状态。
- **阻塞状态（Blocked）**：如果一个线程执行了sleep（睡眠）、suspend（挂起）等方法，失去所占用资源之后，该线程就从运行状态进入阻塞状态。在睡眠时间已到或获得设备资源后（阻塞状态结束后）可以重新进入就绪状态。
