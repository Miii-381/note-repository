...
>
> **2026-09-07 当前仓库复核说明：** 下文保留早期模块逐段解读，但涉及项目回答时以 `D:\Desktop\AI_YunCunChu` 当前代码为准。当前传统业务模块主要是 C，不是全 C++；Docker Compose 是单机三容器、单 Tracker、单 Storage；Nginx 将 13 条 API 路由转到 13 个 FastCGI 端口，并未配置 `upstream` 负载均衡。
>
> 当前注册不是无盐 MD5：前端先做 MD5，服务端生成 16 位十六进制盐并保存 `MD5(salt + 前端MD5)`。Token 仍是用户名到不透明字符串的 Redis 映射，TTL 固定 24 小时，校验时不续期。
>
> 当前普通上传没有“Redis 暂存预期 MD5”，`upload_cgi` 也不重新计算正文摘要；它只核对正文长度，且接口没有 Token。大于 10 MiB 的文件走新增加的分片链路：前端按 10 MiB 顺序上传，Redis Hash `chunk:{md5}` 记录会话，正文片保存在 `/tmp/chunks/{md5}`，合并使用 FastDFS appender API。该链路只检查片文件存在，不校验分片/整体摘要，没有合并锁，单片上传也没有鉴权。
>
> 图片分享后端虽有 `key` 字段，但创建响应不返回 Key，浏览接口也不校验 Key；`dealsharefile` 的取消、转存、公共下载计数以及图片取消分享缺少 Token。当前仓库没有 LRU-K、动态反馈负载均衡、业务线程池、数据库连接池或内存池实现。
>
> AI 代码属于后续项目迭代，暂不进入当前面试回答口径。本文件下文没有展开该迭代。
